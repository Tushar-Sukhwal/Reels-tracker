...
function isReelVideo(video) {
    ...
    const isVisible = rect.width > 150 && // Reduced threshold
        rect.height > 200 &&
    ...
    const isOnReelsPage = window.location.pathname === "/reels" ||
        window.location.pathname === "/reels/";
    ...
    const videoParent = video.closest("article") ||
    ...
    console.log("Instagram video check:", {
        ...
        hasReelIndicators,
        videoSrc: video.src,
        videoParent: videoParent === null || videoParent === void 0 ? void 0 : videoParent.tagName,
        isOnReelsPage,
        isOnSpecificReel,
        isOnHomeFeed,
    });
...
function handleReelStart(video) {
            ...
            if (entry.target === video && !entry.isIntersecting && reelStartTime) {
                ...
                setTimeout(() => {
                    ...
                    if (!entry.isIntersecting &&
                        currentReelElement === video &&
                        reelStartTime) {
                        console.log("Instagram reel went out of view (confirmed), ending tracking");
                        handleReelEnd();
                        cleanup();
...
function handleReelEnd() {
    if (reelStartTime) {
        ...
        if (duration >= 2) {
            ...
            console.log(`✅ Successfully tracked Instagram reel: ${duration} seconds`);
        }
        else {
